# -*- coding: utf-8 -*-
"""
Created on Wed Nov 12 10:01:23 2025
...
# TEST EULER CHARACTERISTIC + MINIMAL SIZE
...
def event_counting_alpha(uncorrected_alpha_matrix,
    ...
    # Tri entre les vrais alphas et les électrons après discrimination de détection bugée
    for props in regionprops(labeled_uncorrected_alpha_matrix, intensity_image=uncorrected_alpha_matrix):
        if props.area > 2:
            is_alpha = (props.euler_number == 1)
            if is_alpha:
    ...
    # Matrice avec uniquement les alphas / compteur d'alphas
    alpha_only_matrix = uncorrected_alpha_matrix * (classification_matrix==1)
    alpha_count = 0
    
    # Si matrice d'alpha uniquement non vide
    if np.any(alpha_only_matrix): 
        
        # Matrice avec les clusters d'alphas labélisés / nombre de cluster
    ...
    # Si matrice d'electrons uniquement non vide
    if np.any(electron_only_matrix):

...
def list_files(folder, recursive=False, extensions=None, fullpath=True, include_hidden=False):
    ...
    -----
    list[pathlib.Path]
        liste de pathlib.Path (si fullpath=True) ou Path objects with only name when fullpath=False
    """
    import os
        ...
        for root, _, filenames in os.walk(folder):
            for name in filenames:
                ...
                if exts and os.path.splitext(name)[1].lower() not in exts:
                    continue
                # Ajouter un pathlib.Path résolu si fullpath True, sinon Path du nom
...
x = np.linspace(100, 2000, 100, dtype=int)  # diviseurs de dt
